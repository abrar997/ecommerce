...
export default function Slider({ title, data, isFirst }) {
  return (
    <div
          ...
          {data.map((slide) => {
            return (
              <SwiperSlide key={slide.id}>
                <div className="group border border-black relative flex flex-col gap-2  p-2 rounded">
                  <img
                    src={slide.image}
                    alt="pic1"
                  ...
                  <div className="bg-[#eeeeeec6] inset-0 absolute flex items-center justify-center opacity-0 h-0 group-hover:opacity-100 group-hover:h-full transition-all duration-300">
                    <button className=" bg-black px-2 py-2 text-white rounded">
                      add to cart
                    </button>
                  </div>
                </div>
